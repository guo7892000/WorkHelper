...
namespace Breezee.Framework.Mini.StartUp
{
    ...
    /// 用户登录界面
    /// </summary>
    public partial class FrmMiniLogin : BaseForm
    {
        #region 全局变量
        ...
        private void FrmLogin_Load(object sender, EventArgs e)
        {
            ...
            cbbUserName.Text = Properties.Settings.Default.LastLoginUserName;
            cbbUserName.AutoCompleteCustomSource.AddRange(Properties.Settings.Default.AutoCompleteUserList.Split(','));
            foreach (string str in Properties.Settings.Default.AutoCompleteUserList.Split(','))
            {
            ...
            txbPassword.Text = Properties.Settings.Default.LastLoginPwd;
            //设置最大的面板背景图片
            this.BackgroundImage = Properties.Resources.WorkHelp_Logo;
            //设置主SQL配置文件路径
            GlobalContext.Instance.SqlMainConfigPath = MiniGlobalValue.Config.SqlMainCofig_Path;
            //设置登录面板的背景图片
        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //验证密码是否正确
                if (!Breezee.Framework.Tool.EncryptHelper.ValidatePassword(password, dtUser.Rows[0][DT_SYS_USER.USER_PASSWORD].ToString()))
                {
                    ShowInfo("用户名不存在或密码错误！", "登录失败");
                    return;
                }
                ...
                //GlobalContext.Instance.LoginUser = user;
                BaseForm._loginUser = new LoginUserInfo(user);
                #region 保存设置信息
                if (chbSavePwd.Checked)
                {
                    Properties.Settings.Default.LastLoginPwd = txbPassword.Text;
                    Properties.Settings.Default.IsRememberPwd = true;
                }
                else
                {
                    Properties.Settings.Default.LastLoginPwd = "";
                    Properties.Settings.Default.IsRememberPwd = false;
                }
                if (ckbAutoLogin.Checked)
                {
                    Properties.Settings.Default.LastLoginUserName = userName;
                    ...
                    if (!isIncludeName)
                    {
                        Properties.Settings.Default.AutoCompleteUserList += userName + ",";
                    }
                }
                else
                {
                    Properties.Settings.Default.LastLoginUserName = "";
                }
