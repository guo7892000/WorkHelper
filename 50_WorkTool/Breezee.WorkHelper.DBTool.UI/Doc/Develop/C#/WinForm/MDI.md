...
再引发 MDI 父窗体的 Closing 事件。 取消 MDI 子窗体的 Closing 事件将不会阻止引发 MDI 父窗体的 Closing 事件；但是，MDI 父窗体的 Closing 					
事件的 CancelEventArgs 参数将设置为 true。 通过将 CancelEventArgs 参数设置为 false 可以强制 MDI 父窗体和所有 MDI 子窗体关闭。					
